...
El patrón que mejor explica qué sitios SÍ tienen medición geodésica cuantitativa (Chile, Brasil) frente a los que no (Colombia, Perú, Bolivia) no es el tamaño del yacimiento ni la severidad del impacto ambiental — es **la existencia de un grupo de investigación universitario local con acceso a InSAR/GRACE y motivación de publicación** (en el caso de Atacama, equipos con acceso a datos SAR multisensor; ver `paises/03-chile.md`). Esta hipótesis fue matizada en la Fase 2 (`paises/08-venezuela.md` §4: la severidad extrema del fenómeno también parece bastar) y en la Fase 3 (`12-fase3-factibilidad-metodologia-atacama.md`: los datos InSAR abiertos ya existen para Uyuni, de modo que la barrera es de procesamiento y publicación, no de captura). Esto sugiere que la intervención de mayor retorno no es tecnológica sino de **alianza institucional**: vincular este repositorio con un grupo geodésico universitario colombiano (ver issue de milestone correspondiente) es más determinante para cerrar la brecha que cualquier mejora de sensor.
